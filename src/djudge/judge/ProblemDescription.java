...
public class ProblemDescription extends AbstractDescription
{
			...
			throws DJudgeXmlException
	{
		try
		{
			problemRoot = JudgeDirs.getProblemsDir() + contestID + "/"
					+ problemID + "/";
	...
	private void parseXML20(Element elem) throws DJudgeXmlException
	{
		...
		String out = problemInfo.programOutputFilename = elem
				.getAttribute("output-file");

		problemInfo.files = new ExecutorFiles(in.length() == 0
				|| in.equalsIgnoreCase("stdin") ? "input.txt" : null, out
				.length() == 0
				|| out.equalsIgnoreCase("stdout") ? "output.txt" : null);
		...
		for (int i = 0; i < groupsCount; i++)
			...
			{
				groups[i] = new GroupDescription(this, i, problemInfo,
						(Element) list.item(i));
				if (ownChecker == null)
				{
					ownChecker = groups[i].getActualValidator();
				}
			}
	}
	...
	private void ParseOldXML(Element elem) throws DJudgeXmlException
	{
		...
		ownLimits.outputLimit = StringTools.StrToMemoryLimit(elem
				.getAttribute("output-limit"));
		ownLimits.memoryLimit = StringTools.StrToMemoryLimit(elem
				.getAttribute("memory-limit"));
		ownLimits.timeLimit = StringTools.strToTimeLimit(elem
		...
		ownChecker = new CheckerDescription(problemInfo.contestID,
				problemInfo.problemID,
				CheckerDescription.StringToType(checker), checkerParam,
				checkerExe);

		groups = new GroupDescription[1];
		groups[0] = new GroupDescription(this, 0, testsCount, problemInfo,
				inputFileMask, outputFileMask, elem.getAttribute("score"));
	}
	...
	private void ParsePcmsXML(Element elem) throws DJudgeXmlException
	{
		...
		String inputFileMask = testset.getAttribute("input-href").replaceAll(
				"tests/", "").replaceAll("###", "%03d")
				.replaceAll("##", "%02d");
		String outputFileMask = testset.getAttribute("answer-href").replaceAll(
				"tests/", "").replaceAll("###", "%03d")
				.replaceAll("##", "%02d");

		String in = problemInfo.programInputFilename = testset
				.getAttribute("input-name");
		String out = problemInfo.programOutputFilename = testset
				.getAttribute("output-name");
		...
		// Limits
		ownLimits = new ExecutorLimits();
		ownLimits.outputLimit = StringTools.StrToMemoryLimit(testset
				.getAttribute("output-limit"));
		ownLimits.memoryLimit = StringTools.StrToMemoryLimit(testset
				.getAttribute("memory-limit"));
		ownLimits.timeLimit = StringTools.strToTimeLimit(testset
				.getAttribute("time-limit"));
		...
		groups = new GroupDescription[1];
		groups[0] = new GroupDescription(this, 0, testsCount, problemInfo,
				inputFileMask, outputFileMask, testset.getAttribute("score"));
	}
		...
		try
		{
			...
			if ("%testlib".equalsIgnoreCase(type))
			{
				...
				if (1 == list.getLength())
				{
					...
					String executableId = exeElement
							.getAttribute("executable-id");
					...
					{
						throw new Exception("wrong executable-id: "
								+ executableId);
					}
				}
				else
	...
	public Document getXML()
	{
		...
		for (int i = 0; i < groupsCount; i++)
		{
			res.appendChild(doc.importNode(groups[i].getXML().getFirstChild(),
					true));
		}
	...
	{
		return problemRoot + "tests/"
				+ groups[groupNumber].tests.get(testNumber).getInputMask();
	}

	public String getJudgeOutputFilepath(int groupNumber, int testNumber)
	{
		return problemRoot + "tests/"
				+ groups[groupNumber].tests.get(testNumber).getOutputMask();
	}
	...
	// possible masks %d, %c, %c%d, %d%c; first mask position is substituted by
	// either digit or letter depending
	// on %c or %d
	protected String substituteMask(String mask, int groupNumber)
	{
		...
		for (int k = 0; k < mask.length(); k++)
		{
			...
			{
				int t = k + 1;
				while (t < mask.length() && mask.charAt(t) != 'd'
						&& mask.charAt(t) != 'c')
					t++;
				if (t >= mask.length())
					break;
		...
		{
			String p1 = new PrintfFormat(mask.substring(0, pos[1]))
					.sprintf(groupNumber + 1);
	...
	public boolean save()
	{
		String problemRoot = JudgeDirs.getProblemsDir() + problemInfo.contestID
				+ "/" + problemInfo.problemID + "/";
		String problemXML = problemRoot + "problem.xml";
		saveXML(problemXML);
		return true;
	...
	public void overrideParams(CheckParams params)
	{
		...
		/*
		 * problemInfo.files = new ExecutorFiles( null == params.inputFilename
		 * || params.inputFilename.length() == 0 ||
		 * params.inputFilename.equalsIgnoreCase("stdin") ? "input.txt" : null,
		 * null == params.outputFilename || params.outputFilename.length() == 0
