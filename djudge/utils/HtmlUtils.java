...
import djudge.acmcontester.server.http.HttpMirror;
...
public class HtmlUtils
{
	...
	{
		sb.append("<html><head><title>Summary/Attempt - Contest</title></head><body>");
	}
	...
	public static String getMonitor(MonitorData data, boolean colored)
	{
	...
	public static String getSubmissions(SubmissionData[] data, Properties params)
	{
		...
		for (int i = 0; i < data.length; i++)
		{
			...
			if (params.containsKey("problem"))
			{
				sb.append("<td>" + data[i].problemID + "</td>");
			}
			if (params.containsKey("user"))
			{
				sb.append("<td>" + data[i].userID + "</td>");
			}
			if (params.containsKey("language"))
			{
				sb.append("<td>" + data[i].languageID + "</td>");
			}
			if (params.containsKey("judgement"))
			{
